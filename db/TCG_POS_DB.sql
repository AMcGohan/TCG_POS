CREATE TABLE `cards` (
  ...
  `foil` boolean,
  `condition` ENUM('Damaged', 'Moderately Played', 'Lightly Played'),
  `count` int,
...
CREATE TABLE `trade_list` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `customer_id` INT,
  `status` enum('In Progress', 'Awaiting Response', 'Finished', 'Cancelled'),
  `credit` decimal(10,2),
  `date_created` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `date_updated` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
...
ALTER TABLE `card_order` ADD FOREIGN KEY (`card_id`) REFERENCES `cards` (`id`);
...
ALTER TABLE `card_trade` ADD FOREIGN KEY (`card_id`) REFERENCES `cards` (`id`);

ALTER TABLE `card_trade` ADD FOREIGN KEY (`price_id`) REFERENCES `card_price` (`id`);

ALTER TABLE `card_price` ADD FOREIGN KEY (`card_id`) REFERENCES `cards` (`id`);
